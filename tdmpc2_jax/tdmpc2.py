...
class TDMPC2(struct.PyTreeNode):
        ...
        *,
        ...
        if self.mpc:
            ...
            if prev_mean is None:
                prev_mean = jnp.zeros((num_envs, self.horizon, self.model.action_dim))
    ...
    def update(
        ...
        *,
        ...
        def world_model_loss_fn(
            ...
            z = self.model.encode(
                jax.tree_map(lambda x: x[0], observations), encoder_params
            )
            zs = zs.at[0].set(z)
    ...
    def estimate_value(
        ...
        Qs, _ = self.model.Q(z, next_action, self.model.value_model.params, key=Q_key)
        Q = Qs.mean(axis=0)
